...
在此我們可以藉由在桌面版開啟 Gaia 的方式手動測試：

1. 複製寫好的 `todomaic` 目錄回 `gaia/apps/todomagic`
2. 在 Gaia 目錄下，執行 `DEBUG=1 make`
3. 以該產生出的設定檔執行 Firefox。例如（指令介面） `firefox -profile ./profile-debug http://todomagic.gaiamobile.org:8080`

讀者要注意上面的指令需要修改：尤其是當你的設定檔跟 Firefox 執行檔路徑不像上述示範時。

這樣應該會直接開啟我們所作的工作清單應用。現在讀者可以透過滑鼠模擬操作去試驗每個功能是否正常：

1. 可以點選新增待辦事項增加一個待辦事項
2. 可以點選待辦事項的按鈕畫上刪除線或取消刪除線
3. 可以點選該按鈕並且選擇「刪除所有」一次刪掉待辦事項

### 撰寫自動化測試

手動測試難免會有遺漏的部份。而程式單元是否正常，也是以手動測試較看不出來的地方。

首先要說明的是，在 FirefoxOS 中的應用程式可以使用任何標準網站所用的單元測試工具進行測試。
這包括諸如 Jasmine 或是 Mochi 這樣知名的測試框架，都可以在 FirefoxOS 上順利執行。

然而如果需要進行整合式測試(Integration Test)，以目前的架構，讀者需要使用 Marionette 這套 Mozilla 所推出，
與 Gecko 核心整合的測試框架。這部份的內容會在後續章節介紹。目前我們先以標準的網頁單元測試工具撰寫測試，
之後再加上更多的測試到我們的
